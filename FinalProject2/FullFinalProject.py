...
#Create output files
    ...
    inventory.damaged()
#Get the the types of product and manufacturer
    types = []
    manufacturers = []
    for item in items:
...
# Prompt the user for input and option to quit
    user_input = None
    while user_input != 'q':
        user_input = input("\nPlease enter an item manufacturer and item type (ie: Lenovo tower) or press 'q' to quit:\n")
        if user_input == 'q':
            break
        else:
#checks input for a match for manufacturer or item type
            ...
            if not selected_manufacturer or not selected_type or bad_input:
                ...
                if matching_items:
                    ...
                    price = item[1]['price']
                    print("Your item is: item ID:{}, {}, {}, ${}\n".format(item_id, man_name, item_type, price))
#output for related item from different manufacturer
                    if similar_items:
                        matched_price = price
                        closest_item = None
                        ...
                        for item in similar_items:
                            if closest_price_diff == None:
                                ...
                                continue
#get item with similar price to initial item
                            ...
                            if price_diff < closest_price_diff:
                                ...
                                item_type = similar_items[item]['item_type']
                                price = similar_items[item]['price']
                        print("You may, also, consider: item ID:{}, {}, {}, ${}\n".format(item_id, man_name, item_type, price))
                else:
                    print("No such item in inventory")
